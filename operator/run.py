...
def handle_legacy(base, token, config, events):
    ...
    for message in cycle['pending'][:3]:
        ...
        if not reply or len(reply)>3000:
            events.append({'feed':'legacy','message_id':message['id'],'status':'model_response_rejected'});continue
        ...
        events.append({'feed':'legacy','message_id':message['id'],'reply_id':posted['id'],'status':'duplicate' if posted.get('duplicate') else 'posted','actual_model':actual,'input_tokens':response.get('prompt_eval_count'),'output_tokens':response.get('eval_count'),'elapsed_seconds':round(time.monotonic()-before,3)})
    return cycle['run_id']
...
def handle_network(base, token, config, events):
    try:
        ...
        if err.code in (400, 404, 503):
            ...
            return None
        raise
    for post in cycle.get('pending', [])[:3]:
        before=time.monotonic()
        response, reply = chat(config, {'contract':'network','post':post})
        if not reply or len(reply)>3000:
            events.append({'feed':'network','parent_id':post['id'],'status':'model_response_rejected'});continue
        actual=response.get('model',config['model'])
...
def main():
    ...
    try:
        ...
        result={'observed_at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'host':os.environ.get('COMPUTERNAME','unknown'),'run_ids':run_ids,'base_url':base,'feed_mode':mode,'elapsed_seconds':round(time.monotonic()-start,3),'events':events,'paid_provider_calls':0,'prompt_sha256':hashlib.sha256(SYSTEM.encode()).hexdigest()}
